...
{   
    ...
    public async Task<string> CreateProduct(Request.CreateProductRequest request)
    {
        ...
        var existingProduct = _dbContext.Products.Where(
            x=> x.Name == request.Name);
        
        var isExistProduct = await existingProduct.AnyAsync();
        if (isExistProduct) throw new Exception("Product name already exists");
        
        var existingSeller = _dbContext.Sellers.Where(x=> x.Id == sellerIdGuid);
        var isExistSeller = await existingSeller.AnyAsync();
        if (!isExistSeller) throw new Exception("Seller is not exists");
        ...
        {
            var productcategory = request.CategoryIds.Select(id => new ProductCategory()
            {
                CategoryId = id,
                ProductId = product.Id
            });
            _dbContext.AddRange(productcategory);
